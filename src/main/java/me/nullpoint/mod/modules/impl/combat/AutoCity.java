...
public class AutoCity extends Module {
	...
	private void doBreak(PlayerEntity player) {
				...
				for (double y : yOffset) {
					for (double x : xzOffset) {
						for (double z : xzOffset) {
							...
							if (isObsidian(offsetPos) && offsetPos.equals(PacketMine.breakPos)) {
								return;
							}
						}
					}
		...
		if (surround.getValue()) {
			...
			for (Direction i : Direction.values()) {
				if (i == Direction.UP || i == Direction.DOWN) continue;
				if (!PacketMine.godBlocks.contains(getBlock(pos.offset(i))) && !(getBlock(pos.offset(i)) instanceof BedBlock)) {
	...
	private boolean isObsidian(BlockPos pos) {
		return (getBlock(pos) == Blocks.OBSIDIAN || getBlock(pos) == Blocks.ENDER_CHEST) && BlockUtil.getClickSideStrict(pos) != null && (!pos.equals(PacketMine.secondPos) || !(mc.player.getMainHandStack().getItem() instanceof PickaxeItem || SilentDouble.INSTANCE.isOn()));
	}
}