...
public class AutoCrystal extends Module {
    ...
    private double getDamage(PlayerEntity target) {
        if (!PacketMine.INSTANCE.obsidian.isPressed() && slowPlace.getValue() && lastBreakTimer.passedMs((long) slowDelay.getValue()) && !(BedAura.INSTANCE.isOn() && BedAura.INSTANCE.getBed() != -1)) {
    ...
    public class CrystalRender {
        ...
        public void onRender3D(Render3DEvent event) {
            ...
            if (text.booleanValue && lastDamage > 0) {
                Render3DUtil.drawText3D(String.format("%.4f", lastDamage), curVec3d, text.getValue());
            }
        }
    }
}
