...
public class THoleKick extends Module {
    ...
    public static THoleKick INSTANCE;
    ...
    public THoleKick() {
        super("THoleKick", Category.Combat);
        INSTANCE = this;
    }
    ...
    public void onUpdate() {
        ...
        if (findBlock(Blocks.REDSTONE_TORCH) == -1 || findClass(PistonBlock.class) == -1) {
            if (autoDisable.getValue()) disable();
            return;
        }
        ...
        if (isTrueFacing(piston, direction)) {
            if (BlockUtil.clientCanPlace(piston)) {
                ...
                if (BlockUtil.getPlaceSide(piston, placeRange.getValue()) != null) {
                    ...
                    for (Direction i : Direction.values()) {
                        if (getBlock(piston.offset(i)) == Blocks.REDSTONE_TORCH) {
                            canPower = true;
                            break;
                        }
                    }
                        ...
                        if (side != null) {
                            ...
                            for (Direction i : Direction.values()) {
                                if (BlockUtil.canPlace(piston.offset(i), placeRange.getValue())) {
                                    ...
                                    int powerSlot = findBlock(Blocks.REDSTONE_TORCH);
                                    doSwap(powerSlot);
                    ...
                    for (Direction i : Direction.values()) {
                        ...
                        CombatUtil.modifyBlockState = Blocks.REDSTONE_TORCH.getDefaultState();
